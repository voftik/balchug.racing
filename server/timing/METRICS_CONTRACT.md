...
```json
{
  ...
  "global": {
    ...
    "lap_and_stint_rules": {
      ...
      "last_timing_stream": "Every canonical r_c LAST source cell is retained in an immutable ledger. Only CONFIRMED_LAP enters the timing stream: an exact source-lap boundary, or a changed/first sparse r_c after the accepted r_i connection/layout baseline. An equal sparse value is UNCONFIRMED because two real equal laps are possible. A >=2-row, >=95%-column dense aggregate block may mark only an equal predecessor as REFRESH_REPEAT; changed/first values in that block remain UNCONFIRMED. r_i is an audit baseline, not a timing event. Pace may combine confirmed LAST events with later explicit-LAPS rows only when the latter retain exact source-cell provenance; tracker-only and pre-provenance legacy rows remain available for tyre/stint chronology but are excluded from timing formulas.",
      ...
      "tyre_age_laps": "The number of laps after the current stint start. With source LAPS it uses the provider-confirmed total. Without source LAPS it is a capture-local count of CONFIRMED_LAP LAST ledger events in that stint; REFRESH_REPEAT, UNCONFIRMED and INVALID cells never age tyres. At confirmed pit_out it is 0. The first partial stint begins at analysis activation and counts from that point until its first pit; it is not presented as a manual correction or an official whole-session total.",
