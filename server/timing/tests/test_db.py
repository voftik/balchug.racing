...
class TimingDatabaseTests(unittest.TestCase):
    ...
    def test_schema_contract_migration_backfills_state_source_and_removes_only_synthetic_rows(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for migration in sorted((Path(__file__).parent.parent / "migrations").glob("00[0-1][0-9]_*.sql")):
                if migration.name.startswith(("0013_", "0014_", "0015_", "0016_", "0017_", "0018_", "0019_")):
                    continue
            ...
            self.assertEqual(migrate(path), ["0013", "0014", "0015", "0016", "0017", "0018", "0019", "0020", "0021", "0022"])
            connection = connect(path)
            try:
    ...
    def test_event_timestamp_guard_migration_clears_only_unsafe_derived_times(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            legacy_migrations.mkdir()
            for migration in sorted((Path(__file__).parent.parent / "migrations").glob("00[0-1][0-9]_*.sql")):
                if migration.name.startswith(("0014_", "0015_", "0016_", "0017_", "0018_", "0019_")):
                    continue
                shutil.copyfile(migration, legacy_migrations / migration.name)
            self.assertEqual(
                migrate(path, directory=legacy_migrations),
            ...
            self.assertEqual(migrate(path), ["0014", "0015", "0016", "0017", "0018", "0019", "0020", "0021", "0022"])
            connection = connect(path)
            try:
    ...
    def test_runtime_checkpoint_migration_preserves_legacy_checkpoint_rows(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(migrate(path), ["0016", "0017", "0018", "0019", "0020", "0021", "0022"])
            connection = connect(path)
            try:
