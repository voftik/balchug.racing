...
class MetricEngineTests(unittest.TestCase):
    ...
    def test_green_same_lap_history_drives_closure_catch_and_projection(self):
        ...
        self.assertEqual(session["battle_lap_trend"]["ahead"]["window_laps"], 3)
        self.assertEqual(session["battle_lap_trend"]["ahead"]["label"], "догоняем")
        self.assertGreater(session["battle_lap_trend"]["ahead"]["catch_laps"], 0)
        self.assertEqual(session["battle_lap_trend"]["behind"]["label"], "отрываемся")
        self.assertIsNone(session["battle_lap_trend"]["behind"]["catch_laps"])
