...
Окружение (systemd EnvironmentFile=/etc/balchug/secrets.env):
  S3_* (обяз.), SESSION_GAP_MIN (деф. 60), MIN_PART_BYTES (деф. 524288),
...
FAIL_DIR = os.path.join(REC_DIR, "failed")
# 60 мин: в гонке 27.06 реальный обрыв эфира длился ~50 мин — порог должен его перекрывать,
# иначе гонка распадается на два видео. Сессии уик-энда обычно разнесены на часы.
...
def finalize(group):
    ...
    try:
        ...
        # не зацикливаемся на битой сессии: убираем её в failed/ и продолжаем
        os.makedirs(FAIL_DIR, exist_ok=True)
        for p in group:
            try:
                shutil.move(p["path"], os.path.join(FAIL_DIR, os.path.basename(p["path"])))
            except OSError:
                pass
        raise
    finally:
